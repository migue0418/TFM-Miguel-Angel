...
const NewMonumentForm = ({
  ...
  const checkUrl = async url => {
    if (!url) return true; // campo vacío ⇒ se salta la validación
    try {
      // En modo no‑cors muchas respuestas serán «opaque» (status 0),
      // consideramos correcto si es ok o opaque.
  ...
  const handleSubmit = async e => {
    ...
    // Comprobamos URLs antes de enviar.
    for (const key of ['url', 'url_maps']) {
      if (!(await checkUrl(formData[key]))) {
        showError(
          `La dirección especificada para "${key}" no responde (no se obtuvo código 200).`
        );
        return;
      }
    }
    ...
    try {
      ...
      const { data } = await api.post(
        '/tourist-guide/management/monuments/create',
        formData,
        {
      ...
      onCreated(data);
      showSuccess('Monumento creado correctamente');
      reset();
      handleClose();
    } catch (err) {
      console.error(err);
      showError('No se pudo crear el monumento');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>Crear Nuevo Monumento</Modal.Title>
      </Modal.Header>
